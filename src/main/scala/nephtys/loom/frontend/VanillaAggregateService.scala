...
import nephtys.loom.protocol.vanilla.solar.Misc.{Dawn, Name, Twilight, Zenith}
...
class VanillaAggregateService {
  ...
  def createNew(id : ID[Solar], name : String) : Unit = internalMap.next(store.+((id, Characters.emptySolar(id, ownermock).copy(name = Name(name)))))
